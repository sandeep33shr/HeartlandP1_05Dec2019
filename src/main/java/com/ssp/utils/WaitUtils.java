...
public class WaitUtils {
	...
	/** The max page load wait. */
	public static int maxPageLoadWait = 60;

	/** The max element wait. */
	...
	public static void waitForSpinner(final WebDriver driver) {
		...
		try {
			Thread.sleep(1000);
			(new WebDriverWait(driver, 60).pollingEvery(500, TimeUnit.MILLISECONDS)
	...
	public static void waitForElement(WebDriver driver, By locator, String msg) {
		(new WebDriverWait(driver, maxPageLoadWait).pollingEvery(200, TimeUnit.MILLISECONDS)
				.ignoring(NoSuchElementException.class, StaleElementReferenceException.class).withMessage(msg))
	...
	public static void waitForExpectedTitle(WebDriver driver, String expectedTitle) {
		(new WebDriverWait(driver, maxPageLoadWait).pollingEvery(200, TimeUnit.MILLISECONDS)
				.ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
				.withMessage("Expected title not matches with the actual title"))
	...
	public static void waitForAlert(final WebDriver driver) {
		(new WebDriverWait(driver, 120).pollingEvery(500, TimeUnit.MILLISECONDS)
				...
				.withMessage("Alert is not present on a webpage")).until(ExpectedConditions.alertIsPresent());
	}
}
